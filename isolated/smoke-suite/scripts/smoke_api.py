...
def build_steps(suffix: str) -> list[Step]:
    ...
    return [
        ...
        Step(
            ...
            expected_statuses=(201,),
            capture=lambda ctx, body: ctx.update({"edge_id": body["id"], "lane_id": body["lanes"][0]["id"]}),
        ),
        Step(
            name="create_bidirectional_edge",
            method="POST",
        ...
        Step(
            name="get_edge",
            method="GET",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}",
            expected_statuses=(200,),
        ),
        Step(
            name="get_edge_editor",
            method="GET",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}/editor",
            expected_statuses=(200,),
        ),
        Step(
            name="patch_edge",
        ...
        Step(
            name="patch_shape",
            method="PATCH",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}/shape",
        ...
        Step(
            ...
            method="POST",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}/recalculate-length",
            expected_statuses=(200,),
        ),
        Step(
            name="replace_lanes",
            method="PUT",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}/lanes",
            body=lambda _ctx: {
                "lanes": [
            ...
            expected_statuses=(200,),
            capture=lambda ctx, body: ctx.update({"lane_id": body["lanes"][0]["id"]}),
        ),
        Step(
            name="patch_lane",
            method="PATCH",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}/lanes/{ctx['lane_id']}",
            body=lambda _ctx: {"allow": "passenger bus taxi", "speed": 12.8},
            expected_statuses=(200,),
        ),
        Step(
            name="apply_road_type",
            method="POST",
            path=lambda ctx: f"/projects/{ctx['project_id']}/edges/{ctx['edge_id']}/apply-road-type",
            body=lambda ctx: {
                "road_type_id": ctx["road_type_id"],
                "speed": 14.4,
                "width": 3.55,
