...
Текущий этап: **Network Constructor + Road Segment Editor + Connection Layer + Intersection Editor + Priority & Signs + Pedestrian Crossings**.
...
- priority/sign layer:
  - роли подходов (`IntersectionApproach.role`, `priority_rank`)
  - валидация схемы приоритета
  - persisted generated знаки (`TrafficSign`)
  - export hints (`priority`, `priority_stop`)
- pedestrian crossing layer:
  - `PedestrianCrossing` как отдельная сущность
  - candidate sides от approaches (`side_key=approach:{approach_id}`)
  - enable/disable crossings
